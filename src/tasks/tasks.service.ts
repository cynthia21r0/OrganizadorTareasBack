...
export class TasksService {
  ...
  async create(dto: CreateTaskDto, requester: RequestUser): Promise<Task> {
    // Regla: solo padre/madre asignan a otros; el resto solo se autoasigna.
    ...
    if (!assignedUser || assignedUser.familyId !== requester.familyId) {
      throw new NotFoundException('El integrante asignado no pertenece a tu familia');
    }
  ...
  // READ - tareas asignadas a un usuario (dentro de la misma familia)
  ...
  async findByUserAndDay(userId: string, day: string, requester: RequestUser): Promise<Task[]> {
  ...
  // READ - todas las tareas de MI familia
  ...
  async update(id: string, dto: UpdateTaskDto, requester: RequestUser): Promise<Task> {
